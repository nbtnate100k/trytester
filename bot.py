...
_BF_DIMS = frozenset({"zip", "state", "city", "bin"})
_BF_SEARCH_HINTS = {
    "bin": "Send a <b>BIN</b> (6 digits, or a digit prefix to match).",
    "zip": "Send a <b>ZIP</b> (digits; partial match works).",
    "state": "Send a <b>state</b> (e.g. <code>NY</code> or text that appears in stock).",
    "city": "Send part of a <b>city</b> name from stock.",
}
_PICK_PAGE_SIZE = 6
_BF_SEARCH_MAX_DISAMBIG = 8


def _apply_filter_dimension_to_userdata(
    context: ContextTypes.DEFAULT_TYPE, dim: str, name: str
) -> None:
...
def _filter_applied_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
...
def _filter_applied_caption(dim: str, name: str, counts: dict) -> str:
    dim_plain = {"zip": "ZIP", "state": "State", "city": "City", "bin": "BIN"}[
        dim
    ]
    tot = int(counts["first"]) + int(counts["second"])
    lbl = (
...
async def _present_filter_applied_edit(
    query, context: ContextTypes.DEFAULT_TYPE, dim: str, name: str, counts: dict
) -> None:
    await query.edit_message_text(
        _filter_applied_caption(dim, name, counts),
        parse_mode=ParseMode.HTML,
        reply_markup=_filter_applied_markup(),
    )


async def _present_filter_applied_reply(
    message, context: ContextTypes.DEFAULT_TYPE, dim: str, name: str, counts: dict
) -> None:
...
def _buy_filter_picklist_matches(
    dim: str,
    pl: list[tuple[str, dict[str, int]]],
    qraw: str,
    ...
    if dim == "bin":
        d = "".join(c for c in q if c.isdigit())
        if not d:
            return []
        for i, (name, cts) in enumerate(pl):
            nb = str(name).strip()
            if nb.startswith(d) or d in nb:
                out.append((i, name, cts))
    elif dim == "zip":
        d = "".join(c for c in q if c.isdigit())
        qu = q.casefold()
        for i, (name, cts) in enumerate(pl):
            nk = str(name).strip()
            if d and (nk.startswith(d) or d in nk):
                out.append((i, name, cts))
            elif not d and qu and qu in nk.casefold():
                out.append((i, name, cts))
    else:
        qu = q.casefold()
        for i, (name, cts) in enumerate(pl):
            nk = str(name).strip()
            if qu in nk.casefold():
                out.append((i, name, cts))
    return out


async def show_buy_filter_dim_chooser(
    query, context: ContextTypes.DEFAULT_TYPE, dim: str
) -> None:
    context.user_data.pop("awaiting_bf_search_dim", None)
    title = _PICK_DIM_TITLE.get(dim, dim)
    await query.edit_message_text(
        f"{title}\n\n<i>Search by typing, or open the full paginated list.</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔎 Type to search", callback_data=f"bf_src:{dim}"
                    ),
                    InlineKeyboardButton(
                        "📋 View full list", callback_data=f"bf_lst:{dim}"
                    ),
                ],
                [
                    InlineKeyboardButton(
...
async def show_buy_filter_search_prompt(
    query, context: ContextTypes.DEFAULT_TYPE, dim: str
) -> None:
    if dim not in _BF_DIMS:
        return
    context.user_data["awaiting_bf_search_dim"] = dim
    hint = _BF_SEARCH_HINTS.get(dim, "Send your search text.")
    title = _PICK_DIM_TITLE.get(dim, "")
    await query.edit_message_text(
        f"{title}\n\n{hint}\n\n<i>Send your reply in this chat.</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "⬅️ Back", callback_data="bf_srch_bk"
                    )
                ],
            ]
        ),
...
async def show_buy_filter_pick(
    ...
    rows.append(
        [
            InlineKeyboardButton("🔎 Type to search", callback_data=f"bf_src:{dim}"),
        ]
    )
    rows.append(
        [InlineKeyboardButton("⬅️ Filter menu", callback_data="bf_cancel_prompt")]
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("awaiting_bf_search_dim"):
        ensure_user(uid)
        dim = context.user_data.get("awaiting_bf_search_dim")
        if dim not in _BF_DIMS:
            context.user_data.pop("awaiting_bf_search_dim", None)
        else:
            q = (update.message.text or "").strip()
            pl = filter_dimension_picklists().get(dim, [])
            matches = _buy_filter_picklist_matches(dim, pl, q)
            if not matches:
                await update.message.reply_text(
                    "No match in stock for that search. Try again, or tap <b>Back</b> on "
                    "the search screen and use <b>View full list</b>.",
                    parse_mode=ParseMode.HTML,
                )
                return
            if len(matches) > 1:
                context.user_data.pop("awaiting_bf_search_dim", None)
                rows: list[list[InlineKeyboardButton]] = []
                for idx, name, cts in matches[:_BF_SEARCH_MAX_DISAMBIG]:
                    tot = cts["first"] + cts["second"]
                    lab = (
                        f"{name} · 1st {cts['first']} · 2nd {cts['second']}"
                        if dim == "bin"
                        else f"{name} · {tot}"
                    )
                    rows.append(
                        [
                            InlineKeyboardButton(
                                _short_button_label(lab),
                                callback_data=f"bfx:{dim}:{idx}",
                            )
                        ]
                    )
                rows.append(
                ...
                extra = ""
                if len(matches) > _BF_SEARCH_MAX_DISAMBIG:
                    extra = (
                        f"\n\n<i>Showing {_BF_SEARCH_MAX_DISAMBIG} of {len(matches)} — "
                        "type a more specific search to see other matches.</i>"
                    )
                await update.message.reply_text(
                    f"<b>Several matches — pick one:</b>{extra}",
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup(rows),
                )
                return
            idx, name, cts = matches[0]
            context.user_data.pop("awaiting_bf_search_dim", None)
            _apply_filter_dimension_to_userdata(context, dim, name)
            await _present_filter_applied_reply(
                update.message, context, dim, name, cts
            )
            return
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("bf_src:"):
        dim = data.split(":", 1)[1] if ":" in data else ""
        if dim in _BF_DIMS:
            await show_buy_filter_search_prompt(query, context, dim)
        return

    if data.startswith("bf_lst:"):
        dim = data.split(":", 1)[1] if ":" in data else ""
        if dim in _BF_DIMS:
            context.user_data.pop("awaiting_bf_search_dim", None)
            await show_buy_filter_pick(query, context, dim, 0)
        return

    if data == "bf_srch_bk":
        dim = context.user_data.pop("awaiting_bf_search_dim", None)
        if dim in _BF_DIMS:
            await show_buy_filter_dim_chooser(query, context, dim)
        return
    ...
    if m_bfx:
        ...
        context.user_data.pop("awaiting_bf_search_dim", None)
        _apply_filter_dimension_to_userdata(context, dim, name)
        await _present_filter_applied_edit(query, context, dim, name, counts)
        return
    ...
    if data == "bf_zip":
        await show_buy_filter_dim_chooser(query, context, "zip")
        return

    if data == "bf_city":
        await show_buy_filter_dim_chooser(query, context, "city")
        return

    if data == "bf_state":
        await show_buy_filter_dim_chooser(query, context, "state")
        return

    if data == "bf_bin":
        await show_buy_filter_dim_chooser(query, context, "bin")
        return
